...
def fire_events(sym, df15, start, end):
    """Pump_Doctor fires: sigup (vol>=avg_up & green) AND flow>0, liquid."""
    ...
    sum_up = pd.Series(upv).rolling(LOOKBACK).sum().to_numpy()
    sum_dn = pd.Series(dnv).rolling(LOOKBACK).sum().to_numpy()
    cnt_up = pd.Series(green.astype(float)).rolling(LOOKBACK).sum().to_numpy()
    ...
    for i in range(40, n):
        if not (start <= ct[i] <= end):
            continue
        if (green[i] and np.isfinite(avg_up[i]) and v[i] >= avg_up[i] and flow[i] > 0 and
...
def main():
    print("STANDALONE Pump_Doctor CATCHER (15m directional-volume) + EMA60_20 exit\n")
