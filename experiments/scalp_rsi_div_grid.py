#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Grid: 5m MACD zero-cross scalp + RSI threshold (rows) x DIVERGENCE lookback (cols).
RSI thr at cross: none/>50/>55/>60/>65. Divergence net over W candles before the cross
(macd[i-1]>macd[i-W] and close[i-1]<close[i-W]): none/3/4/5/6. Reports WR matrix, trade
count, and the best cell. 2025-01, TP+3/SL-2, cost 0.25%, $2000, 10%/trade, 10 conc.
Run: python experiments/scalp_rsi_div_grid.py
"""
...
def main():
    print(f"Grid: MACD + RSI threshold x divergence lookback — {MONTH}\n", flush=True)
    ...
    # per signal: (rsi_val, {W: div_bool}, ent_t, xt, ret)
    sigs = []
    for c in coins:
        ...
        for i in range(20, n-1):
            ...
            if not (rising_at(m15, s15, ent_t) and rising_at(m1h, s1h, ent_t)
                ...
                continue
            div = {W: (m5[i-1] > m5[i-W] and c5[i-1] < c5[i-W]) for W in (3, 4, 5, 6)}
            ...
            sigs.append((float(r5[i]), div, ent_t, xt, ret))
        del df5; gc.collect()

    print(f"إشارات MACD: {len(sigs)}\n", flush=True)

    def cell(rt, W):
        sel = []
        for rv, div, et, xt, rr in sigs:
            if rt is not None and not (rv > rt):
                continue
            if W is not None and not div[W]:
                continue
            sel.append((et, xt, rr))
        nwin, nloss, prof, ret = sim_month(sel)
        return nwin, nloss, prof, ret, nwin+nloss

    # WR matrix
    hdr = "div\\RSI" + "".join(f"{('بلا' if t is None else '>'+str(t)):>9}" for t in RSI_THR)
    print("##### مصفوفة WR% (الصفوف=دايفرجنس، الأعمدة=عتبة RSI) #####")
    print(hdr)
    best = None
    for W in DIV_W:
        row = f"{('بلا' if W is None else str(W)+'شمعة'):<8}"
        for rt in RSI_THR:
            nwin, nloss, prof, ret, ntk = cell(rt, W)
            wr = nwin/ntk*100 if ntk else 0
            row += f"{wr:>7.0f}%" if ntk else f"{'-':>8}"
            if ntk >= 25 and (best is None or wr > best[0]):
                best = (wr, ret, prof, ntk, rt, W)
        print(row)
    print("\n##### عدد الصفقات #####")
    print(hdr)
    for W in DIV_W:
        row = f"{('بلا' if W is None else str(W)+'شمعة'):<8}"
        for rt in RSI_THR:
            _, _, _, _, ntk = cell(rt, W)
            row += f"{ntk:>8}"
        print(row)
    if best:
        wr, ret, prof, ntk, rt, W = best
        print(f"\nأفضل خلية (≥25 صفقة): RSI>{rt} + دايفرجنس {W}شموع → WR {wr:.0f}%، عائد {ret:+.1f}%، {ntk} صفقة، ربح {prof:+,.0f}$")
    print("الهدف: WR فوق ~45% مع عدد صفقات كافٍ. (شهر واحد فقط — يحتاج تأكيداً متعدّد الأشهر.)")
