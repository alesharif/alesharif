#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""EMA7200+5% entry, exit TP+3% / SL-3% (first touch on intrabar high/low). 3:1 R:R,
...
def main():
    ...
    del raw
    coins = list(vol)
    print(f"عملات النطاق: {len(coins)} | TP+3% / SL-3% مقابل الأساس\n", flush=True)
    TPL = []; TPN = []; TPW = []; BL = []
    ...
    print("-"*64)
    print(f"{'TP+3% / SL-3%':<26}{tp.mean():>+7.0f}%{np.median(tp):>+7.0f}%{(tp>0).mean()*100:>7.0f}%{np.mean(TPW):>5.0f}%{np.mean(TPN):>7.0f}")
    print(f"{'الأساس (تحت EMA7200)':<26}{bl.mean():>+7.0f}%{np.median(bl):>+7.0f}%{(bl>0).mean()*100:>7.0f}%")
    print("-"*64)
    print("تعادل TP3/SL3 = WR 50% (1:1)")
    print("DONE_TP3SL3.")
