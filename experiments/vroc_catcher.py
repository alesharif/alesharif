...
def fire_events(sym, df15, start, end):
    """Normalized volume-rate-of-change (Pump Catcher): vroc_norm > 40, price+vol rising."""
    ...
    if n < 120:
        return []
    LEN = 96                                  # 1 day on 15m (480 5m-bars)
    mav = pd.Series(v).rolling(LEN).mean().to_numpy()
    mav1 = np.concatenate([[np.nan], mav[:-1]])
    diff = mav - mav1
    vol_inc = diff > 0
    val_inc = np.concatenate([[False], c[1:] > c[:-1]])
    with np.errstate(invalid="ignore", divide="ignore"):
        vroc = np.where(vol_inc & val_inc & (mav1 > 0), diff * 100.0 / mav1, 0.0)
    vroc = np.nan_to_num(vroc)
    historic_max = np.maximum.accumulate(np.maximum(vroc, 10.0))   # floor 10
    vroc_norm = vroc / historic_max * 100.0
    atr = atr_w(h, l, c, 14)
    qvavg = pd.Series(qv).rolling(20).mean().shift(1).to_numpy()
    out = []
    for i in range(LEN + 5, n):
        if not (start <= ct[i] <= end):
            continue
        if (vroc_norm[i] > 40.0 and
...
def main():
    print("STANDALONE VROC CATCHER (15m normalized volume rate-of-change) + EMA60_20 exit\n")
