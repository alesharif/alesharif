#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""DAILY Stochastic bullish crossover (%K crosses above %D), slow stochastic 14,3,3.
Daily execution + DCA [40-60-100-100-100] + trailing 20%. Two variants: pure cross |
+ slow line %D > 10. Run:  python experiments/daily_stoch.py
"""
...
def main():
    print("DAILY Stochastic bullish crossover (14,3,3) + DCA + trailing 20%\n", flush=True)
    ...
    for sym, df in raw.items():
        ...
        g = df.set_index("dt")
        d = pd.DataFrame({"h": g["high"].resample("D").max(), "l": g["low"].resample("D").min(),
                          "c": g["close"].resample("D").last(), "v": g["volume"].resample("D").sum(),
                          "t": g["time"].resample("D").last()}).dropna()
        dc = d["c"].to_numpy(); dh = d["h"].to_numpy(); dl = d["l"].to_numpy()
        dt_ = d["t"].to_numpy(); ddv = (d["c"]*d["v"]).to_numpy(); n = len(dc)
        if n < 60:
            df.drop(columns=["dt"], inplace=True, errors="ignore"); continue
        ll = pd.Series(dl).rolling(14).min().to_numpy(); hh = pd.Series(dh).rolling(14).max().to_numpy()
        rng = np.where((hh-ll) == 0, 1e-9, hh-ll)
        kfast = 100*(dc-ll)/rng
        kslow = pd.Series(kfast).rolling(3).mean().to_numpy()       # slow %K
        dslow = pd.Series(kslow).rolling(3).mean().to_numpy()       # %D
        T = df["time"].to_numpy(); H = df["high"].to_numpy(float); L = df["low"].to_numpy(float); C = df["close"].to_numpy(float)
        for i in range(20, n-1):
            if not (np.isfinite(dslow[i]) and kslow[i-1] <= dslow[i-1] and kslow[i] > dslow[i]):
                continue
            if np.nanmean(ddv[max(0, i-30):i]) <= LIQ_MIN:
                continue
            d_above10 = dslow[i] > 10
            P0 = float(dc[i]); ent_t = int(dt_[i]); yr = int(pd.Timestamp(ent_t, unit="ms").year)
