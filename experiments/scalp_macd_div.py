#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Add BULLISH DIVERGENCE to the 5m MACD zero-cross scalp: at the cross, the 3 candles
PRECEDING it show MACD values RISING while price FALLS. Keeps multi-TF MACD filter +
vol gate. Compare baseline vs +divergence (strict monotonic / net) on 2025-01.
TP+3/SL-2, cost 0.25%, $2000 independent, 10%/trade, 10 concurrent. Reuses cached 5m.
Run: python experiments/scalp_macd_div.py
"""
...
def main():
    print(f"MACD zero-cross + BULLISH DIVERGENCE (3 candles before) — {MONTH}\n", flush=True)
    ...
    sigs = []   # (div_strict, div_net, ent_t, xt, ret)
    for c in coins:
        ...
        for i in range(20, n-1):
            ...
            if not (rising_at(m15, s15, ent_t) and rising_at(m1h, s1h, ent_t)
                ...
                continue
            # divergence over the 3 candles preceding the cross: i-3, i-2, i-1
            ds = (m5[i-1] > m5[i-2] > m5[i-3]) and (c5[i-1] < c5[i-2] < c5[i-3])      # strict
            dn = (m5[i-1] > m5[i-3]) and (c5[i-1] < c5[i-3])                          # net
            ...
            sigs.append((ds, dn, ent_t, xt, ret))
        del df5; gc.collect()

    print(f"إشارات MACD (قبل الدايفرجنس): {len(sigs)}\n", flush=True)
    print(f"{'الإعداد':<24}{'صفقات':>7}{'رابحة':>7}{'خاسرة':>7}{'WR':>6}{'عائد%':>9}{'ربح$':>9}")
    print("-"*62)
    for key, lab in [("base", "أساس (MACD فقط)"), ("net", "+ دايفرجنس صافٍ"), ("strict", "+ دايفرجنس صارم")]:
        if key == "base":
            sel = [(et, xt, rr) for ds, dn, et, xt, rr in sigs]
        elif key == "net":
            sel = [(et, xt, rr) for ds, dn, et, xt, rr in sigs if dn]
        else:
            sel = [(et, xt, rr) for ds, dn, et, xt, rr in sigs if ds]
        nwin, nloss, prof, ret = sim_month(sel)
        ntk = nwin+nloss; wr = nwin/ntk*100 if ntk else 0
        print(f"{lab:<24}{len(sel):>7}{nwin:>7}{nloss:>7}{wr:>5.0f}%{ret:>+8.1f}%{prof:>+8,.0f}$")
    print("-"*62)
    print("الدايفرجنس الصعودي: قيم MACD ترتفع بينما السعر ينزل في الشموع الثلاث قبل التقاطع.")
    print("\nDONE_DIV.", flush=True)
