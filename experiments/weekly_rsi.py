#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""WEEKLY RSI rising after a decline: RSI(14) crosses UP through an oversold threshold
(was <=thr, now >thr). Daily execution + DCA [40-60-100-100-100] + trailing 20%.
Sweep thresholds 20/22/25/27/30. Run: python experiments/weekly_rsi.py
"""
...
CAP = 3.0; TRAIL = 0.20; START = 2000.0; THRS = [20, 22, 25, 27, 30]
...
def main():
    print("WEEKLY RSI rising from oversold + DCA + trailing 20%\n", flush=True)
    raw = PIT.prefetch_universe(PIT.list_all_usdt_symbols(), ms(S), ms(E), log=lambda *a: None)
    rows = []      # (ent_t, yr, rr, rprev, rcur)
    for sym, df in raw.items():
        ...
        for w in range(16, nw-1):
            # candidate cross only if RSI turned up (was falling, now rising)
            if not (r[w] > r[w-1]):
                continue
            ...
            rows.append((ent_t, yr, rr, float(r[w-1]), float(r[w])))
    ...
    # signal at threshold T: RSI crossed UP through T this week (was <=T, now >T)
    for thr in THRS:
        sel = [r for r in rows if r[3] <= thr and r[4] > thr]
        report(sel, f"RSI يعبر صعوداً من تحت {thr}")
    print("⚠️ متفائل بانحياز البقاء.")
    print("\nDONE_WRSI.", flush=True)
